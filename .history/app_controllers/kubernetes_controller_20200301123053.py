...
class KubernetesController():
    ...
    def manageKubernetesServicePod(self):
        try:
            subprocess.Popen([f"kubectl {self.kubectlAction} d-f {self.currentDirectory}/app_controllers/infrastructure/kubernetes-deployments/services/{self.serviceName}/01_cluster-role-{self.googleKubernetesComputeCluster}-{self.serviceName}-{self.userName}.yml >> /dev/null 2>&1"], shell=True).wait()
